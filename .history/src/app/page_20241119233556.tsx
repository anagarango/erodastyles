					...
					<div style={{width:"80%"}}>
						...
						<div style={{display:"flex", flexWrap:"wrap", gap:10}}>
							{posts.toReversed().map((o,i)=>{
								if(i < 4){
									return(
										<div id="recentposts" onClick={()=>r.push(`/blog/${10-i}`)} style={{width:"45%", padding:"20px", cursor:"pointer"}}>
				...
				<div>
					...
					<div style={{display:"flex", flexWrap:"wrap", gap:"20px", padding:"30px 0"}}>
						{musicalInfluences.map((o,i)=>(
							<div onClick={()=>r.push(`/musicalinfluences/${i}`)} style={{width:"calc(50% - 10px)", padding:"20px", height:"350px", borderRadius: 15, position:"relative", cursor:"pointer", backgroundImage:`url(/musicalinfluences/${o.cover})`, backgroundSize:"cover", backgroundRepeat:"no-repeat", backgroundPosition:"center"}}>
								<h4 id="heading" className="musicalinfluences" style={{width:"calc(100% - 40px)", backgroundColor:"rgba(255,255,255,0.9)", position:"absolute", padding:"30px", bottom:"5%", fontSize:"25px", borderRadius:5}}>{o.title}</h4>
							</div>
						))}
					</div>
				</div>
