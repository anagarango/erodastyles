  ...
  return (
    <div style={{display:"flex", flexDirection:"column", alignItems:"center"}}>
			...
			<div style={{width:"100%", padding:"50px", maxWidth:"1250px"}}>
				...
				<div>
					<h1 style={{fontSize:"26px", fontWeight:"600" ,borderBottom:"1px solid #DBCDCD"}}>HARRY'S MUSICAL INFLUENCES</h1>
					<div style={{display:"flex", flexWrap:"wrap", gap:"20px", padding:"30px 0"}}>
						{musicalInfluences.map((o,i)=>(
							<div onClick={()=>r.push(`/musicalinfluences/${i}`)} style={{width:"calc(50% - 10px)", padding:"20px", height:"350px", borderRadius: 15, position:"relative", backgroundImage:`url(/musicalinfluences/${o.cover})`, backgroundSize:"cover", backgroundRepeat:"no-repeat", backgroundPosition:"center"}}>
				...
				<div>
					<h1 style={{fontSize:"26px", fontWeight:"600" ,borderBottom:"1px solid #DBCDCD"}}>HARRY'S RECENT PROJECT</h1>
					<div style={{display:"flex", flexWrap:"wrap", gap:"20px", padding:"30px 0"}}>
						{recentprojects.map((o,i)=>(
							<div onClick={()=>r.push(`/blog/${10-i}`)} style={{display:"flex", width:"50%", padding:"20px"}}>
								<div>
									<h4 id="heading" style={{fontSize:"25px"}}>{o.title}</h4>
									<p style={{padding:"20px 0", textAlign:"center"}}>{o.snippet}</p>
								</div>
								<img src={`/recentprojects/${o.cover}`} style={{width:"50%"}}/>
							</div>
						))}
					</div>
				</div>
